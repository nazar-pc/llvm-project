...
### Targeting an interpreter rather than a processor

RISC-V is increasingly used as a portable execution target for software rather than as a description of hardware: blockchain virtual machines, sandboxes, deterministic replay environments and emulators all run RISC-V code through an interpreter or a simple JIT. `generic-interpreter`, `generic-interpreter-rv32` and `generic-interpreter-rv64` exist for that consumer. They are not processors, and the model behind them is not a pipeline.

What such a consumer charges for is different in kind from what silicon charges for:

- **The number of instructions executed.** Every guest instruction is one dispatch, so halving the instructions a program executes roughly halves the time it takes. A scheduling model describes how instructions overlap, not how many of them there are, so this objective has no expression in an ordinary processor model.
- **The cost of each dispatch, which is not uniform.** A load pays for address arithmetic, a bounds check and a memory access; a division runs a much longer handler. This is real, and it is deliberately *not* expressed as a scheduling model — see below.
- **Data-dependent branches.** A guest conditional branch becomes a host indirect jump whose target depends on guest data, and the host branch predictor cannot learn it the way it learns its own branches. Nothing is predictable, which is why `getPredictableBranchThreshold()` is one and why jumps are marked expensive for these CPUs.
- **Code footprint, but not through an instruction cache.** A pre-decoding interpreter stores one fixed-size slot per instruction, and a JIT pays compile time. Neither behaves like an instruction cache, so the pressure that normally bounds inlining and unrolling does not apply in the usual way — but the decoded form is walked as *data*, so a large enough function trades an instruction cache it does not have for a data cache it does.

These CPUs deliberately use the same scheduling model as `generic-rv32` and `generic-rv64`, and contribute tuning only. A model of an interpreter would be a single serial dispatch slot whose occupancy is the per-dispatch cost, which is an accurate description and precisely what a scheduler cannot use: when occupancy equals latency there is no slack, so there is no gap to fill and no reason to move an instruction, while the scheduler remains free to lengthen a live range for other reasons. Measured on a 5x5-limb field multiply, such a model left all fifty partial products live at once for 27 spill stores, where the hardware model interleaves each product with its consumer and spills nothing; the workload ran 10% slower than with no scheduler at all, and 18% slower than with the hardware model. A spill is a guest memory access, which is among the most expensive dispatches an interpreter performs.

Static code size and the number of instructions executed are separate objectives here, and optimizing the first can hurt the second, so a change should be judged by counting executed instructions rather than by measuring size. Counting them is not sufficient either: a spill is one dispatch like any other, but it is a guest memory access, and the decoded stream grows with the code.

Inlining and unrolling are therefore left alone by default, rather than being raised as the absence of an instruction cache would suggest. Both are available as knobs for tuning a particular consumer (`-riscv-interpreter-inlining-threshold-multiplier` and `-riscv-interpreter-max-unroll-count`, both 1 by default), and raising them does win on small benchmarks. It loses badly elsewhere: inlining a callee whose loop has a high register demand makes the trip count a constant, the loop is then unrolled, and a body that already needed every register spills. Measured on an ed25519 field inversion, a multiplier of 2 turned a 248-byte function into 17436 bytes containing 454 spill stores per call, and the workload ran 64% slower despite executing slightly fewer instructions.

These CPU names carry no ISA of their own beyond the base integer instruction set, exactly like `generic-rv32` and `generic-rv64`, so the extensions still come from `-march`. `-mtune=generic-interpreter` selects the tuning alone. The tuning is also available directly as the `interpreter-target` subtarget feature, for targets that can only reach a feature string.
