...
static bool simplifySwitchLookup(SwitchInst *SI, IRBuilder<> &Builder,
  ...
  if (UseUnsignedRange && SignedRange.isAllOnes() && HasDefaultResults &&
    ...
    // The signed range spans every value the condition can take, so its table
    // needs no range check. That is worth more than a smaller table, as long
    // as we would actually build it. The size saturates for wide types, where
    // such a table is never built.
